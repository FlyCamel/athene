...
public class PreloadDataCache<K, V> extends SimpleCache<K, V> {
    ...
    private static String                   TAG                           = "PreloadDataCache";
    ...
    public CacheObject<V> get(K key) {
        ...
        if (object != null) {
            /** if saved image version is 0 or not equal whit current app version, delete saved image **/
            if (object.getVersion() == 0 || object.getVersion() != SystemUtils.getVersionCode(context)) {
                if (object.getData() instanceof String) {
                    deleteFile((String)object.getData());
                    super.remove(key);
                    object = null;
                }
            }
